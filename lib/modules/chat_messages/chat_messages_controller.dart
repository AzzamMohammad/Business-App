...
class ChatMessagesController extends GetxController{
  ...
  Future<void> GetNewMessages()async{
    ...
    while(!State){
      NewMessages = await chatMessageServer.GetNewMessages(Token, IsFoundation, ChatId, NextPageUrl);
  ...
  Future<void> SendNewMessage()async{
    ChatMessage  NewMessage = ChatMessage(id: 500, isFoundation: IsFoundation, userIsRead: IsFoundation == 0 ? 1:0,
      foundationIsRead: IsFoundation == 1 ? 1:0, createdAt: DateTime.now()
      , message: MessageTextContent ,video: LoadingVideo,photo: LoadingImage,messageTextReplay:CommentMessage.value!=1000000000000? ChatMessagesList[CommentMessage.value].message:null,messagePhotoReplay:CommentMessage.value!=1000000000000? ChatMessagesList[CommentMessage.value].photo:null,messageVideoReplay:CommentMessage.value!=1000000000000? ChatMessagesList[CommentMessage.value].video:null,messageReplay: CommentMessage.value!=1000000000000?CommentMessage.value:-1);
    NewMessage.MessageState('sending');
    ChatMessagesList.insert(0,NewMessage);
    NumberOfMessagesInList(ChatMessagesList.length);
    CommentMessage(1000000000000);
    MessageFieldController.clear();
    State = await chatMessageServer.SendNewMessage(Token , ChatId , IsFoundation , MessageTextContent , LoadingVideo , LoadingImage , IdOfCommentMessage);
    if(State == false){
  ...
  Future<void> ReSendMessage(int index)async{
    ...
    State = await chatMessageServer.SendNewMessage(Token , ChatMessagesList[index].id , IsFoundation , ChatMessagesList[index].message  , ChatMessagesList[index].video  , ChatMessagesList[index].photo  , ChatMessagesList[index].messageReplay);
    if(State == false){
  ...
  Future<void> BlockTheChat(BuildContext context , int index)async{
    State = await chatMessageServer.BlockTheChat(Token,IsFoundation,context,index);
    Message = chatMessageServer.Message;
  }

  Future<void> UnBlockTheChat(BuildContext context , int index)async{
    State = await chatMessageServer.UnBlockTheChat(Token,IsFoundation,context,index);
    Message = chatMessageServer.Message;
  }
